...
evalNoControlSims <- function(d.list, s.list, pars, times, yini){
  ...
  resilience <- matrix(NA, n.r,2)
  colnames(resilience) <- c("exp.decay", "quantile")
  ...
  for (i in 1:length(d.list)){
    ...
    for (j in 1:length(s.list)){
      ...
      stability <- energyStabilityMetricsNoControl(euc.dist, 
                                                   times, 
                                                   pars$event.t)
